...
# Progress is written to the information stream rather than to the host, so that a caller can
# capture or redirect it. That stream is silent by default, and these messages are the script's
# output rather than optional detail, so turn it on.
...
if ($runExitCode -ne 0) {
	...
	Write-Warning "  gh run rerun $runId --repo $repoFullName --failed"
	exit 1
}
