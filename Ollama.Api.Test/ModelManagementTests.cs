	...
	: Test(fixture, testOutputHelper)
{
	...
	public async Task ModelLifecycle_CreateCopyDelete_Succeeds()
	{
		...
		await foreach (var update in OllamaClient.CreateModelStreamAsync(createRequest, default))
		{
	...
	public async Task ShowModel_ReturnsModelInfo()
	{
		...
		var response = await OllamaClient.Models.ShowAsync(request, default);
		response.Should().NotBeNull();
		response.Name.Should().Be(request.Name);
	}

	[Fact]
	public async Task PullModel_ReturnsSuccess()
	{
	...
	[Fact]
	public async Task PushModel_ReturnsSuccess()
	{
	...
	{
		var response = await OllamaClient.Models.GetTagsAsync(default);
		response.Should().NotBeNull();
	...
	public async Task ShowModel_MissingModel_Returns404()
	{
		...
		{
			await OllamaClient.Models.ShowAsync(request, default);
		});
	...
	public async Task CopyModel_MissingSource_Returns404()
	{
		...
		var ex = await Assert.ThrowsAsync<Refit.ApiException>(async () =>
		{
			await OllamaClient.Models.CopyAsync(request, default);
		});
		ex.StatusCode.Should().Be(System.Net.HttpStatusCode.NotFound);
	}
	...
	[Fact]
	public async Task CreateModel_InvalidBaseModel_Returns404()
	{
		...
		var ex = await Assert.ThrowsAsync<Refit.ApiException>(async () =>
		{
			await OllamaClient.Models.CreateAsync(request, default);
		});
		ex.StatusCode.Should().Be(System.Net.HttpStatusCode.NotFound);
	}

	[Fact]
	public async Task CreateModelStream_InvalidBaseModel_Returns404()
	{
		var request = new CreateModelRequest
		{
			Name = "test-invalid-create:fake",
		...
		var ex = await Assert.ThrowsAsync<Refit.ApiException>(async () =>
		{
			...
			await foreach (var update in OllamaClient.CreateModelStreamAsync(request, default))
			{
